...
# ----------------------------------------------------------------------------
# Cấu hình đường dẫn lưu trữ demo & Tên mô hình AI
# ----------------------------------------------------------------------------
DATA_DIR = "lost_found_data"
IMAGES_DIR = os.path.join(DATA_DIR, "images")
DB_FILE = os.path.join(DATA_DIR, "items.json")
MULTILINGUAL_TEXT_MODEL_NAME = "sentence-transformers/clip-ViT-B-32-multilingual-v1"
...
@st.cache_resource(show_spinner="Đang nạp mô hình Text đa ngôn ngữ...")
def load_multilingual_text_model(model_name: str = MULTILINGUAL_TEXT_MODEL_NAME):
...
def get_text_embedding(text_model, text: str):
    """Trích xuất vector 512 chiều chuẩn hóa L2 từ Multilingual Text Transformer."""
    vec = text_model.encode(text)
...
def build_prompt_text(name: str, desc: str) -> str:
    """Chuẩn hóa văn bản tiếng Việt cho Multilingual Text Transformer."""
    name = (name or "").strip()
    desc = (desc or "").strip()
    if name and desc:
        return f"{name}. Mô tả: {desc}"
    elif name:
        return name
    elif desc:
        return desc
...
def recompute_all_embeddings(
    ...
    """Tự động tính lại toàn bộ embeddings với tùy chọn tiền xử lý YOLO thông minh và Multilingual CLIP Text."""
    for cat in ["lost", "found"]:
        for item in db.get(cat, []):
            ...
            item["txt_embedding"] = (
                get_text_embedding(text_model, prompt) if (prompt and text_model is not None) else None
            )
    db["current_model"] = model_name
    db["current_text_model"] = MULTILINGUAL_TEXT_MODEL_NAME
...
def load_db(
    ...
    """Nạp database và tự động kiểm tra đồng bộ model/yolo/multilingual."""
    if os.path.exists(DB_FILE):
        with open(DB_FILE, "r", encoding="utf-8") as f:
            data = json.load(f)

        needs_sync = False
        if clip_model is not None and processor is not None and model_name is not None and text_model is not None:
            if (
                data.get("current_model") != model_name
                or data.get("current_text_model") != MULTILINGUAL_TEXT_MODEL_NAME
        ...
        if needs_sync:
            data = recompute_all_embeddings(
                data, clip_model, processor, text_model, model_name, yolo_model, use_yolo, strict_lf_only
            )
        return data
    return {
        "lost": [],
        ...
        "current_text_model": MULTILINGUAL_TEXT_MODEL_NAME,
        "use_yolo": use_yolo,
...
with st.spinner("Đang khởi tạo các mô hình AI..."):
    ...
    text_model = load_multilingual_text_model(MULTILINGUAL_TEXT_MODEL_NAME)
    yolo_model = load_yolo_model("yolov8n.pt") if enable_yolo else None

db = load_db(clip_model, processor, text_model, selected_model_name, yolo_model, actual_use_yolo, strict_lf_only)
...
if st.sidebar.button("🔄 Tái tạo toàn bộ Embeddings"):
    with st.spinner("Đang tính toán lại vector embedding cho toàn bộ cơ sở dữ liệu..."):
        db = recompute_all_embeddings(
            db, clip_model, processor, text_model, selected_model_name, yolo_model, actual_use_yolo, strict_lf_only
        )
...
st.caption(
    "Kiến trúc kết hợp: **YOLOv8 Smart Object Crop** + **CLIP ViT-B/16 (Ảnh)** + "
    "**Multilingual CLIP (Text Tiếng Việt)** + **Center HSV** + **Spatio-Temporal Gating**"
)
    ...
    with st.form("lost_form", clear_on_submit=True):
        ...
        if submitted:
            if image_file is None or not name:
                ...
                prompt = build_prompt_text(name, desc)
                txt_emb = (
                    get_text_embedding(text_model, prompt) if prompt else None
                )
    ...
    with st.form("found_form", clear_on_submit=True):
        ...
        if submitted2:
            if image_file2 is None or not name2:
                ...
                txt_emb2 = (
                    get_text_embedding(text_model, prompt2) if prompt2 else None
                )
...
with tab3:
    ...
    if not db["lost"]:
        ...
        st.info(
            ...
            "2. **CLIP ViT-B/16 Vision:** Trích xuất đặc trưng không gian sâu với 196 patches (gấp 4 lần B/32), phân biệt chi tiết nhỏ hiệu quả.\n"
            "3. **Multilingual CLIP Text Encoder:** Mã hóa ngữ nghĩa tiếng Việt tự nhiên trực tiếp, giải quyết rào cản từ vựng tiếng Anh của CLIP gốc.\n"
            "4. **Center HSV Color Histogram:** Giải quyết triệt để vấn đề mù màu và attribute binding của CLIP gốc.\n"
            "5. **Spatio-Temporal Constraints:** Ràng buộc nhân quả thời gian ($t_{found} \\ge t_{lost}$) và đối chiếu vị trí địa lý giúp loại trừ các ứng viên phi logic.\n"
            "6. **Dynamic Adaptive Weighting:** Không bao giờ phạt oan điểm khi dữ liệu thiếu mô tả văn bản hoặc địa điểm."
        )
